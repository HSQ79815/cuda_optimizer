...
__global__ void Reduce(const T *src, T *dest, size_t N)
{
...
template <typename T>
bool Check(const T *s1, const T *s2, size_t N, float epsilon)
{
...
template <typename T,size_t THREADS_PER_BLOCK>
void ReduceCpu(const T *src, T *dest, size_t N)
{
...
int main(int argc, char **argv)
{
    ...
    if(!Check(&cpu_dest[0],&device_to_cpu[0],dest_N,epsilon)){
        ...
        std::cout << device_to_cpu[0] << "\t"<< device_to_cpu[1] << "\t"<< device_to_cpu[2] << "\t"<< device_to_cpu[3] << "\t"
                    << device_to_cpu[4] << "\t"<< device_to_cpu[5] << "\n";
    }
