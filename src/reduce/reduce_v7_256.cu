...
{
    __shared__ T sdata[WARP_SIZE];
    unsigned int tid = threadIdx.x;
    unsigned int idx = tid + THREADS_PER_BLOCK * blockIdx.x;

    unsigned int warp_id = tid / WARP_SIZE;
    unsigned int lane_id = tid & (WARP_SIZE - 1);
    ...
    if (warp_id == 0) {
        sum = (tid < THREADS_PER_BLOCK / WARP_SIZE) ? sdata[tid] : T{};
        sum += __shfl_down_sync(0xffffffff, sum, 16, 32);
        sum += __shfl_down_sync(0xffffffff, sum, 8, 16);
        sum += __shfl_down_sync(0xffffffff, sum, 4, 8);
        sum += __shfl_down_sync(0xffffffff, sum, 2, 4);
...
int main(int argc, char** argv)
{
    ...
    LOG(INFO) << device_to_cpu[0] << "\t" << device_to_cpu[1];
    // float epsilon = 1e-2;
    ...
    float bandwidth = (N + 1) * sizeof(type) / elapsedTime * 1000 / (1 << 30);
